...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (uMsg == WM_PAINT)
    {
        ...
        HFONT hFont = CreateFont(65 * scaleY, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE,
        ...
        // paint the numbers in squares
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
            {
                Rect.top = i * sz.x / 3 + paddingX;
                Rect.bottom = (i + 1) * sz.x / 3 - paddingX;
                Rect.left = j * sz.y / 3 + paddingY;
                Rect.right = (j + 1) * sz.y / 3 - paddingY;
                FillRect(hdc, &Rect, mat[i][j] == 9 ? (HBRUSH)(COLOR_WINDOW + 1) : (HBRUSH)(1));
                SetTextColor(hdc, RGB(0x00, 0x00, 0x00));
                SetBkMode(hdc, TRANSPARENT);
                wchar_t s[2];
                _itow_s(mat[i][j], s, 2, 10);
                if (mat[i][j] < 9)
    ...
    else if (uMsg == WM_LBUTTONDOWN)
    {
        ...
        int xPos = HIWORD(lParam),
            yPos = LOWORD(lParam),
            i = xPos * 3 / (rect.bottom - rect.top),
            j = yPos * 3 / (rect.bottom - rect.top);
        if (wcscmp(buttonLabel, L"Calc") == 0 && i < 3 && j < 3)
        {
            // move squares manually
            finished = move(i, j);
        ...
        if (yPos < rectButton2.right && yPos > rectButton2.left)
            if (xPos < rectButton2.bottom && xPos > rectButton2.top)
                ...
                {
                    // implement moves after calculation
                    if (seq.empty())
                        if (A_star())
                    ...
                    {
                        memcpy(mat, seq.top().mat, sizeof(int) * 9);
                        seq.pop();
                        if (seq.empty())
                            wcscpy_s(buttonLabel, L"Over");
                        steps--;
        ...
        if (yPos < rectSelect1.right && yPos > rectSelect1.left)
            ...
            {
                // select the first heuristic function
                method = 0;
                initialize();
                wcscpy_s(buttonLabel, L"Next");
                A_star();
        ...
        if (yPos < rectSelect2.right && yPos > rectSelect2.left)
            ...
            {
                // select the second heuristic function
                method = 1;
                initialize();
                wcscpy_s(buttonLabel, L"Next");
                A_star();
        ...
        if (yPos < rectSelect3.right && yPos > rectSelect3.left)
            ...
            {
                // select the third heuristic function
                method = 2;
                initialize();
                wcscpy_s(buttonLabel, L"Next");
                A_star();
        ...
        if (yPos < rectSelect4.right && yPos > rectSelect4.left)
            ...
            {
                // select the fourth heuristic function
                method = 3;
                initialize();
                wcscpy_s(buttonLabel, L"Next");
                A_star();
